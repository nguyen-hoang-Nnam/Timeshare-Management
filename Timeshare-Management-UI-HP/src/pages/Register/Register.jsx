...
const Register = () => {
    ...
    try {
      if (validateForm()) {
        ...
        if (response.status === 200 && response.data.isSucceed) {
          alert("Register Success");
          navigation("/login"); // Chuyển hướng đến trang đăng nhập sau khi đăng ký thành công
        } else {
  ...
  const validateForm = () => {
    ...
    // ... (giữ nguyên phần kiểm tra hợp lệ của bạn)

    return true; // Trả về true nếu form hợp lệ
  };
